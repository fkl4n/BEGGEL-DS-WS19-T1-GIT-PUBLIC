...
for (path in c("../Workspace/PostgreSQL.csv", "../Workspace/MySql.csv", "../Workspace/MongoDB.csv")){
  ...
  correlation_per_file = df %>% 
    group_by(file) %>%
    summarize(correlation=cor(change,fileSize), changeCount = length(file)) %>%
  ...
  example_flies_with_the_biggest_correlation = correlation_per_file %>% 
    filter(0.9 <= correlation) %>% 
    arrange(changeCount)  %>%
    top_n(10)
  example_flies_with_the_biggest_correlation <- unlist(as.vector(example_flies_with_the_biggest_correlation['file']))
  
   extract_specific_data3 <- extract_specific_data2 %>% 
    group_by(file) %>%
    filter(file %in% example_flies_with_the_biggest_correlation) %>% 
    mutate(fileSize = cumsum(change))  %>%
    arrange(file, timestamp)

  #extract_specific_data3
   
  #Plot für Files mit hoher Korrelations.
  print(ggplot(data = extract_specific_data3) + geom_point(aes(x=fileSize, y=change, colour=file)))  
